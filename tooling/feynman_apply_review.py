...
def apply(review_bundle: Path, review_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    review_input = _load_object(review_bundle / "review-input.json")
    manifest = _load_object(review_bundle / "review-manifest.json")
    ...
    actual_input_sha = hashlib.sha256((review_bundle / "review-input.json").read_bytes()).hexdigest()
