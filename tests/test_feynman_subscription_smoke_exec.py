...
class SubscriptionSmokeExecTests(unittest.TestCase):
    def setUp(self):
        ...
        self.preflight_patch.start(); self.auth_patch.start()
        self.tmp = tempfile.TemporaryDirectory()
        self.base = Path(self.tmp.name)
        self.candidate = self.base / "candidate"
        self.evaluator = self.base / "evaluator"
        self.control = self.base / "control-codex-home"
        ...
        self.remote = self.control / "environments.toml"
        self.remote.write_text('default = "candidate"\ninclude_local = false\n', encoding="utf-8")
        ...
        self.smoke_spec.write_text(json.dumps(smoke), encoding="utf-8")
        ...
        self.job.write_text(json.dumps({
            ...
            "paths": {
                ...
                "control_codex_home": str(self.control.resolve()),
            },
        }), encoding="utf-8")
    ...
    def tearDown(self):
        self.auth_patch.stop(); self.preflight_patch.stop(); self.tmp.cleanup()
...
'''
        if failed_event:
            code += 'print(json.dumps({"type":"turn.failed","error":{"message":"fake"}}))\n'
        elif success:
    ...
    def _run(self, **overrides):
        kwargs = dict(
            plan_path=self.plan, smoke_spec_path=self.smoke_spec, ordinal=1,
            evaluator_case_path=self.eval_case, runner_job_path=self.job,
            boundary_profile_path=self.profile, remote_environment_path=self.remote,
    ...
    def test_success_uses_scrubbed_env_and_frozen_controls(self):
        ...
        self.assertEqual(result["conversation"]["thread_id"], "thread-smoke-1")
        self.assertFalse(result["privacy"]["stderr_nonempty"])
        ...
        argv = state["argv"]
        for flag in ("--json", "--ephemeral", "--strict-config", "--ignore-rules", "--skip-git-repo-check"):
        ...
        self.assertFalse(Path(state["env"]["TMPDIR"]).exists())
        self.assertEqual(len(AUTH_CALLS), 1); self.assertEqual(len(PREFLIGHT_CALLS), 1)
    ...
    def test_control_config_and_remote_path_are_exact(self):
        ...
        with self.assertRaises(ValueError): self._run()
        (self.control / "config.toml").write_text(CONFIG_TEXT, encoding="utf-8")
        other = self.base / "elsewhere.toml"; other.write_text("x", encoding="utf-8")
    ...
    def test_smoke_spec_reasoning_and_raw_bytes_are_bound(self):
        plan = json.loads(self.plan.read_text()); plan["model_reasoning_effort_policy"] = "high"
        self.plan.write_text(json.dumps(plan), encoding="utf-8")
        with self.assertRaises(ValueError): self._run()
        plan["model_reasoning_effort_policy"] = "model-default"; plan["smoke_spec_sha256"] = "f" * 64
        self.plan.write_text(json.dumps(plan), encoding="utf-8")
        with self.assertRaises(ValueError): self._run()

    def test_mock_model_and_candidate_project_config_are_rejected(self):
        job = json.loads(self.job.read_text()); job["versions"]["model"] = "mock-model"
        self.job.write_text(json.dumps(job), encoding="utf-8")
        with self.assertRaises(ValueError): self._run()
        job["versions"]["model"] = "gpt-test"; self.job.write_text(json.dumps(job), encoding="utf-8")
        (self.candidate / ".codex").mkdir()
        with self.assertRaises(ValueError): self._run()
    ...
    def test_evaluator_and_control_home_must_be_disjoint(self):
        nested = self.evaluator / "control"; nested.mkdir()
        (nested / "config.toml").write_text(CONFIG_TEXT, encoding="utf-8"); (nested / "environments.toml").write_text("x", encoding="utf-8")
        job = json.loads(self.job.read_text()); job["paths"]["control_codex_home"] = str(nested.resolve())
        self.job.write_text(json.dumps(job), encoding="utf-8")
    ...
    def test_nonzero_and_failed_trace_do_not_promote_result(self):
        ...
        with self.assertRaises(ValueError): self._run()
        out = self.evaluator / "exec-1"; self.assertTrue((out / "codex-trace.jsonl").exists()); self.assertFalse((out / "subscription-exec-result.json").exists())
        self.assertFalse((out / ".control-tmp").exists())
        out.rename(self.evaluator / "failed-nonzero")
        self._write_fake_codex(success=True, failed_event=True)
        with self.assertRaises(ValueError): self._run()
...
class SubscriptionSmokeExecSchemaTests(unittest.TestCase):
    def test_schema_tracks_privacy_and_reasoning_policy(self):
        ...
        self.assertIn("smoke_spec_sha256", digests["required"]); self.assertNotIn("stderr_sha256", digests["properties"])
