...
class SubscriptionPreflightTests(unittest.TestCase):
    def setUp(self):
        ...
        mount_paths={"candidate_dir":str(self.candidate.resolve()),"evaluator_dir":str(self.eval.resolve()),"source_repo":str(ROOT.resolve()),
          "ephemeral_home":str(self.home.resolve()),"codex_home":str(self.codex.resolve()),"temp_dir":str(self.temp.resolve()),"real_home":str(self.real.resolve()),"control_codex_home":str(self.control.resolve())}
        ...
        use_native_profile(self.profile,mount_paths)
    ...
    def test_symlink_control_home_rejected(self):
        for p in self.control.iterdir():p.unlink()
        self.control.rmdir();target=self.real/"other";target.mkdir()
        try:
