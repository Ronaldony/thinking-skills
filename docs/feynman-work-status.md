...
| FYN-04 평가 격리 | **부분 완료 — exec/patch/synthetic-auth reference 통과** | boundary profile/canary/inspect + control-plane Codex → network-none stdio exec-server + synthetic credential separation | 실제 외부 model-service credential/응답 + 실제 평가 run에서 동일 계약 재검증 필요 |
...
| FYN-06 자동 검사/회귀 | **완료(구조)** | full unittest + structural/Docker/Codex/remote-exec/remote-patch/synthetic-auth workflows | 실제 model-run regression은 FYN-08 이후 추가 |
| FYN-07 평가 데이터 보강 | **개발 세트 완료** | 18 public-development cases, 20 predefined hard failures, controls | 독립 작성 held-out final set 필요 |
| FYN-08 통제된 신·구 비교 | **미실행** | frozen plan/conditions/result-v3/aggregator-v3 준비 | authorized external model control plane, repeats, semantic review 필요 |
...
- **credential-free remote exec E2E reference 성공**
  - control/tool Codex version 동일
  ...
  - control plane dummy auth-like env key가 remote tool env에 전파되지 않음
  - tool output이 다음 model request로 실제 반환됨
  - final agent message까지 왕복
  - Docker inspect/profile 일치
- **remote apply-patch E2E reference 성공**
  - installed Codex bundled catalog에서 실제 `apply_patch_tool_type=freeform` 지원 metadata만 선택
  - unsupported model에 patch capability를 임의 추가하지 않음
  - selected remote environment에서 patch output round-trip 후 remote exec가 patch marker를 읽음
- **synthetic-auth separation reference 성공**
  - run-scoped synthetic bearer가 control-plane request에 존재함을 digest로 확인
  - candidate remote tool env/file/argv에 synthetic credential material이 없음
  - 보존 artifact exact-byte leak scan 통과
  - 실제 account/API credential은 사용하지 않음
...
- 실제 평가 run마다 boundary canary + runner attestation + runner-job link를 새로 보존
- 행동 평가에 실제 사용되는 모든 candidate-accessible tool 유형의 boundary regression
- 실제 multi-turn model run에서 same-thread continuity 검증

중요: reference 성공은 **검증한 transport/tool/auth architecture의 실행 가능성**을 입증하지만 실제 model-service 인증 성공, 계정 정책, rate limit, 모델 품질을 입증하지 않는다.

## FYN-05 세부 상태 — analysis-result v3

canonical analysis chain은 다음이다.
...
analysis-result v3 migration 검증 head `cb85c21266608a0f3011af07e432ea4d31e27898`에서:

- full unittest diagnostic: **285 tests, OK**
...
이후 schema/docs 변경은 다시 CI로 검증한다. 테스트 수는 새 schema regression 추가로 증가할 수 있으므로 과거 285를 현재 최종 수로 고정하지 않는다.
...
사람/runner 운영자가 해야 하는 것은 secret 값을 채팅이나 repository에 쓰는 일이 아니라, 승인된 secret store 또는 runner environment에서 **control-plane 전용 credential**을 사용할 수 있게 하는 것이다. candidate tool env/file/argv에는 credential을 전달하지 않는다.

현재 검증된 auth contract:
...
여기서 env-key **이름**만 artifact에 기록하며 실제 값은 기록하지 않는다. 인증 설정 전까지의 schema/doc/orchestration/negative regression 작업은 사람 개입 없이 진행 가능하다.
...
[완료] mock exec + remote patch + synthetic-auth boundary references
  ...
  → canonical real-run orchestration / smoke-plan artifact 준비
  → 실제 model-service control-plane 인증 설정  ← 첫 사람/권한 개입 지점
  → 동일 boundary profile inspect + canary
  → 1~2개 public-development real-model smoke run
  → runner-job / attestation / link / trace / result-v3 확인
